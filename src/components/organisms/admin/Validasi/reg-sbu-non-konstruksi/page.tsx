...
const ValidasiSBUN = () => {
  ...
  const fetchSbUs = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:8000/api/sbun/pending"
      );
  ...
  const handleDownload = async (userId: number) => {
    try {
      const response = await axios.get(
        `http://localhost:8000/api/sbun/${userId}/download`,
        {
          responseType: "blob",
        }
      );
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="p-6 rounded-lg shadow-2xl w-[500px] relative bg-gradient-to-r from-blue-600 to-purple-700 text-white">
              <h3 className="text-2xl font-extrabold mb-4">
                Detail SBU Non Konstruksi <br /> {detailData.nama_perusahaan}
              </h3>
              <div className="space-y-4">
                ...
                <p>
                  ...
                  {detailData.nama_klasifikasi || "-"}
                </p>
                <p>
                  <strong>Sub Klasifikasi :</strong>{" "}
                  {detailData.nama_sub_klasifikasi || "-"}
                </p>
                <p>
                  <strong>Rekening :</strong> {detailData.nama_rekening || "-"}
                </p>
                <p>
                ...
                <p>
                  ...
                  {detailData.status_aktif === "active"
                    ? "Aktif"
                    : "Tidak Aktif"}
                </p>
              </div>
