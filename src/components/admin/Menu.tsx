...
import { FaTachometerAlt, FaNewspaper, FaImages, FaCalendarAlt, FaUsers, FaSignOutAlt, FaQuestionCircle } from "react-icons/fa"; 
...
const Sidebar = ({ onLogout }) => {
  ...
  return (
    <aside className="fixed inset-y-0 flex-wrap items-center justify-between w-full p-4 my-4 overflow-y-auto antialiased transition-transform duration-200 bg-gray-900 border-0 shadow-lg max-w-64 ease-nav-brand z-50 rounded-2xl">
        ...
        <ul className="flex flex-col pl-0 mb-0">
          {menuItems.map((item) => (
            <li className="w-full mt-1" key={item.title}>
              <Link href={item.href} className="flex items-center p-3 text-white text-sm rounded-lg hover:bg-gray-700 transition-colors">
          ...
          <li className="w-full mt-1">
            <button onClick={toggleDropdown} className="flex items-center p-3 text-white text-sm rounded-lg hover:bg-gray-700 transition-colors w-full">
              <FaUsers className="text-xl" />
              <span className="ml-3">Anggota</span>
              <span className={`ml-auto transition-transform ${isDropdownOpen ? "rotate-180" : ""}`}>
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isDropdownOpen ? "max-h-40" : "max-h-0"}`}>
              <Link href="/admin/anggota/sbu-konstruksi" className="flex items-center p-2 hover:bg-gray-700 text-white text-sm">
                <FaUsers className="text-xl" />
                <span className="ml-3">SBU-Konstruksi</span>
              </Link>
              <Link href="/admin/anggota/sbu-non-konstruksi" className="flex items-center p-2 hover:bg-gray-700 text-white text-sm">
                <FaUsers className="text-xl" />
      ...
      <div className="mt-4">
        <button onClick={onLogout} className="flex items-center p-3 mt-auto hover:bg-gray-700 text-white">
