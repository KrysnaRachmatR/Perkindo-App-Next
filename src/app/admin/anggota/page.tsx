...
const AdminAnggotaPage = () => {
  ...
  try {
    const response = await axios.get("http://localhost:8000/api/sbu-konstruksi", {
      headers: {
        'Authorization': `Bearer ${'17|scWbuhiy4x0RSHhJKAy9SXhYG1LBGxZyEZ842gCre495dad0'}`, // Ganti with your token
      }
    });
  ...
  try {
    ...
    setError(error.response ? error.response.data.message : "Failed to save data");
  } finally {
                ...
                {anggota.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="border px-4 py-2">
                      ...
                      <button onClick={() => handleDelete(item.id)} className="bg-red-500 text-white px-2 py-1 rounded ml-2">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
